...
pub trait ServiceDiscovery {
    async fn resolve(&self, svc:&ServiceDetails, sender: mpsc::Sender<(String,SocketAddr)>);
...
type ResolveListeners = Arc<RwLock<MultiMap<String, mpsc::Sender<(String,SocketAddr)>>>>;
...
impl DynamoDBServiceDiscovery{
	...
	if let Ok(region) = rusoto_signature::Region::from_str(&region){
		...
		Ok(Ok(hst)) => hst,
		Ok(Err(_)) => { warn!("Can't get hostname");"EMPTY".to_string()}
		Err(_) => { warn!("Can't get hostname");"EMPTY".to_string()}	    
	    };
	    let announced_services = Arc::new(RwLock::new(Vec::new()));
	    let ip = get_ip_addr().map(|s| s.to_string());
	    let myself = Self{ client,table,port,listeners,instance_name,fqdn,announced_services,ip};
	...
	expression_attribute_values.insert(":cutoff_time".to_string(),rusoto_dynamodb::AttributeValue{
...
//		debug!("output => {:?}",output);
		if let Some(items) = output.items{
			...
			.filter(|f|  f.0.is_some() && f.1.is_some() && f.2.is_some() && f.3.is_some())
			.map(|f| (f.0.unwrap(), f.1.unwrap(), f.2.unwrap(), f.3.unwrap()))
			.map(|f| (f.0.s.as_ref(),f.1.s.as_ref(),f.2.n.as_ref(),f.3.s.as_ref() ))
			.filter(|f| f.0.is_some() && f.1.is_some() && f.2.is_some() && f.3.is_some())
			.map(|f| (f.0.unwrap(), f.1.unwrap(), f.2.unwrap(), f.3.unwrap()))
			.map(|f| ServiceDesc{ service_name: f.0.to_string(), ip: f.1.to_string(), port:f.2.to_string(), domain: f.3.to_string()})
			.collect();
			...
			for_each(|svc_desc| {
				...
				for (i,channel) in channels.iter_mut().enumerate(){
				    match channel.try_send((domain.clone(),sock)){
					Ok(())=>{},
