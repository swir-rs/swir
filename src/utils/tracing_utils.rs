...
use opentelemetry::{
    api,
    api::{
	context::propagation::text_propagator::HttpTextFormat,
	Provider
    },	
    sdk,
};
...
pub fn from_bytes(span:Span, value: &[u8])->Span{
    let propagator = api::TraceContextPropagator::new();
    let mut carrier = std::collections::HashMap::new();	carrier.insert("traceparent".to_string(),String::from_utf8_lossy(value).to_string());
    let ctx = propagator.extract(&carrier);
    span.set_parent(&ctx);
    span   	
}
...
pub fn init_tracer(config:&Swir) -> Result<(), Box<dyn std::error::Error>> {
	...
	if let Some(open_telemetry) = &cfg.open_telemetry{
		...
		with_agent_endpoint(format!("{}:{}",open_telemetry.collector_address,open_telemetry.collector_port).parse().unwrap()).with_process(opentelemetry_jaeger::Process {
