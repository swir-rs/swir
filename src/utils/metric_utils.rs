...
use opentelemetry::sdk::{
    metrics::{selectors, aggregators, PushController},
    export::metrics::{AggregatorSelector,Aggregator}
};
...
pub fn init_metrics(config: &Swir) -> Result<(MetricRegistry, Option<PushController>), Box<dyn std::error::Error + Send + Sync + 'static>> {
	...
	rss: meter.u64_value_recorder("process_rss").with_description("Resident Set Size: number of pages the process has in real memory in pages").init(),
	threads: meter.u64_value_recorder("process_threads").with_description("Number of threads in this process").init(),
	utime: meter.u64_value_recorder("process_utime").with_description("Amount of time that this process has been scheduled in user mode, measured in clock ticks").init(),
	stime: meter.u64_value_recorder("process_stime").with_description("Amount of time that this process has been scheduled in kernel mode, measured in clock ticks").init(),
	cutime: meter.u64_value_recorder("process_cutime").with_description("Amount of time that this process's waited-for children have been scheduled in user mode, measured in clock ticks").init(),
	cstime: meter.u64_value_recorder("process_cstime").with_description("Amount of time that this process's waited-for children have been scheduled in kernel mode, measured in clock ticks").init(),
    };
